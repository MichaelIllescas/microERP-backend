...
public class GetCurrentUserController {
    ...
    public UserResponse getCurrentUser(@AuthenticationPrincipal Jwt jwt) {
        ...
        log.info("📥 [GET /users/me] Solicitando perfil del usuario con keycloakId={}", keycloakId);

        UserResponse user = getCurrentUserUseCase.getCurrentUser(keycloakId);

        log.info("✅ Perfil obtenido: id={} email={}", user.getId(), user.getEmail());
        return user;
    }
}
